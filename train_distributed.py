...
def main():
    ...
    logs = {}
    start_epoch = 0
    net = WMul(num_classes=num_classes, M=config.num_attentions, net=config.net, pretrained=False)
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        # 开始训练
        train(epoch=epoch, logs=logs, data_loader=train_loader, net=net, feature_center=feature_center, optimizer=optimizer, pbar=pbar)
        
        # 验证训练效果
        validate(logs=logs, data_loader=validate_loader, net=net,pbar=pbar, epoch=epoch)
...
def train(**kwargs):
    ...
    net.train()
    batch_len = len(data_loader)  # 一个batch的图片的数量 = 全部图片的数量/batch_size
    for i, (X, y) in enumerate(data_loader):
        # 调整学习率 
        float_iter = float(i) / batch_len  # 在该batch中，的第几张图片
        adjust_learning(optimizer, epoch, float_iter)
        now_lr = optimizer.param_groups[0]['lr']
        
        optimizer.zero_grad()

        # obtain data for training x是输入，y是预测
        X = X.cuda()
        y = y.cuda()
        ...
        with torch.no_grad():
            ...
            epoch_raw_acc = raw_metric(y_pred_raw, y)   # 原始准确率
            epoch_crop_acc = crop_metric(y_pred_aug, y_aug) # 裁剪准确率
            epoch_drop_acc = drop_metric(y_pred_aux, y_aux) # 随机抹除准确率

        # end of this batch
        batch_info = 'Loss {:.4f}, Raw Acc ({:.2f}, {:.2f}), Aug Acc ({:.2f}, {:.2f}), Aux Acc ({:.2f}, {:.2f}), lr {:.5f}'.format(
            epoch_loss, epoch_raw_acc[0], epoch_raw_acc[1], epoch_crop_acc[0], epoch_crop_acc[1], epoch_drop_acc[0], epoch_drop_acc[1], now_lr)
