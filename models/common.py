...
class Conv_MP(nn.Module):
    def __init__(self, c1, c2):  # ch_in, ch_out
        super(Conv_MP, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(c1, c2, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(c2),
            #nn.ReLU(inplace=True),   # 默认版本
            nn.LeakyReLU(0.1),         # LeakyReLU版本
            #nn.SiLU(),
        )
...
class FusedMBConv(nn.Module):
    def __init__(self, c1, c2, stride, stochastic_depth_prob: float):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(c1, c3, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(c3),
            nn.SiLU(),         
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(c3, c2, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(c2),       
        )
...
class SE(nn.Module):
    def __init__(self, channel, expend_channel):
        super(SE, self).__init__()
        # Squeeze操作
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.fc1 = nn.Conv2d(channel, expend_channel, 1)
        self.act = nn.SiLU()
        self.fc2 = nn.Conv2d(expend_channel, channel, 1)
        self.scale_act = nn.Sigmoid()
        
    def forward(self, x):
        y = self.avgpool(x)
        y = self.act(self.fc1(y))
        y = self.scale_act(self.fc2(y))
        return y * x

class DwConv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k, s, p=1, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(DwConv, self).__init__()
        
        self.conv = nn.Conv2d(c1, c2, k, s, 1, groups=g, bias=False)
        if k==5:
            self.conv = nn.Conv2d(c1,c2,k,s,2,groups=g,bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU()
...
class MB(nn.Module):
    def __init__(self, c1, c2, c3, stochastic_depth_prob): # c1-输入 c2-输出 c3-隐藏层
...
class MBConv(nn.Module):
    def __init__(self, c1, c2, c3, power, kernel_size, stride, stochastic_depth_prob): # c1-输入 c2-输出 c3-隐藏层 power-拓展倍率
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(c1, c_m, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(c_m),
            nn.SiLU(),         
        )
        
        self.conv2 = nn.Sequential(
            DwConv(c_m,c_m,kernel_size,stride,1,c_m) 
        )
        
        self.se = SE(c_m, c3)
        
        self.conv3 = nn.Sequential(
            nn.Conv2d(c_m, c2, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(c2),
        )
...
class Conv_Hswish(nn.Module):
    ...
    def __init__(self, c1, c2, stride):
        super(Conv_Hswish, self).__init__()
...
class MobileNet_Block_drop(nn.Module):
    def __init__(self, inp, oup, hidden_dim, kernel_size, stride, use_se, use_hs, stochastic_depth_prob):
        super(MobileNet_Block_drop, self).__init__()
        assert stride in [1, 2]

        self.identity = stride == 1 and inp == oup
        self.stochastic_depth = torchvision.ops.StochasticDepth(stochastic_depth_prob,"row")
        self.dropblock = torchvision.ops.DropBlock2d(stochastic_depth_prob, 3, inplace = True)
        self.dropout = nn.Dropout(p=stochastic_depth_prob)
        ...
        if inp == hidden_dim:
            self.conv = nn.Sequential(
                ...
                # torchvision.ops.DropBlock2d(stochastic_depth_prob, 3, inplace = True),
                h_swish() if use_hs else nn.SiLU(), # nn.LeakyReLU(0.1), # nn.ReLU(inplace=True), #nn.SiLU(),
                # Squeeze-and-Excite
                SELayer(hidden_dim) if use_se else nn.Sequential(),
                #SE(hidden_dim,hidden_dim//4) if use_se else nn.Sequential(),
                # pw-linear
...
class MobileNet_Block_shuffle(nn.Module):
    def __init__(self, inp, oup, hidden_dim, kernel_size, stride, use_se, use_hs):
        super(MobileNet_Block_shuffle, self).__init__()
        assert stride in [1, 2]
...
class MobileNet_Block(nn.Module):
    def __init__(self, inp, oup, hidden_dim, kernel_size, stride, use_se, use_hs=True):
        ...
        if inp == hidden_dim:
            self.conv = nn.Sequential(
                ...
                nn.Conv2d(hidden_dim, hidden_dim, kernel_size, stride, (kernel_size - 1) // 2, groups=hidden_dim, bias=False),
                nn.BatchNorm2d(hidden_dim),
                h_swish() if use_hs else nn.SiLU(), # nn.LeakyReLU(0.1),
                # Squeeze-and-Excite 尝试用全卷积代替fc
                SELayer(hidden_dim) if use_se else nn.Sequential(),
                # SE(hidden_dim,hidden_dim//4) if use_se else nn.Sequential(),
                # pw-linear
                nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=False),
